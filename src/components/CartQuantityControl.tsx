    ...
    if (quantity <= 0) {
        return (
            <TouchableOpacity
                ...
                className={`bg-[#4CAF50] ${compact ? 'h-9' : 'h-10'} rounded-xl flex-row items-center justify-center ${(disabled || loading) ? 'opacity-70' : ''}`}
            >
                {loading ? (
    ...
    return (
        <View className={`bg-[#E8F5E9] border border-[#C8E6C9] rounded-xl ${compact ? 'h-9 px-2' : 'h-10 px-3'} flex-row items-center justify-between`}>
            <TouchableOpacity
                onPress={onDecrement}
                disabled={disabled || loading}
                className={`w-7 h-7 rounded-full items-center justify-center ${(disabled || loading) ? 'bg-[#F1F8E9]' : 'bg-white'}`}
            >
