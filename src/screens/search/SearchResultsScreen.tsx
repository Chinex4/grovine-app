...
export const SearchResultsScreen = ({ navigation, route }: any) => {
    const [query, setQuery] = useState(route?.params?.query || 'Fruits');
    ...
    const results = searchResponse?.data && Array.isArray((searchResponse as any).data.data)
            ...
            <View className="flex-1">
                ...
                {isLoading ? (
                    ...
                    <View className="flex-1 items-center justify-center px-6">
                        <Text className="text-[#FF5252] font-satoshi font-bold text-center">Failed to fetch search results</Text>
                        <TouchableOpacity
                            onPress={() => navigation.goBack()}
                    ...
                    <View className="flex-1 items-center justify-center px-6">
                        ...
                        <Text className="text-[#9E9E9E] font-satoshi font-bold mt-4 text-[16px]">No results found for "{query}"</Text>
                        <Text className="text-[#BDBDBD] font-satoshi text-center mt-2">Try searching for something else</Text>
                    </View>
                ) : (
                                ...
                                >
                                    <View className="relative">
                                        <View className="bg-white rounded-2xl overflow-hidden border border-gray-50 shadow-sm">
                                            <Image
                                                source={item.image?.url ? { uri: item.image.url } : item.media?.url ? { uri: item.media.url } : require('../../assets/images/shakshuka.png')}
                                                className="w-full h-32"
                                                resizeMode="cover"
                                            />
                                            {item.baskets_left !== undefined && (
                                                <View className="absolute bottom-2 left-2 bg-white/80 px-2 py-0.5 rounded-md flex-row items-center">
                                                    ...
                                                    <Text className="text-[#424242] text-[6px] font-bold ml-1">{item.baskets_left} Baskets Left</Text>
                                                </View>
                                            )}
                                        </View>
                                    </View>
                                    ...
                                    <View className="mt-3">
                                        ...
                                        <TouchableOpacity
                                            onPress={(e) => {
                                                e.stopPropagation();
                                                // Assuming adding to cart logic or navigate to cart
                                            }}
